...
class TimerView: UIView {
    ...
    override func layoutSubviews() {
        guard readyInPercents != nil, let proxy = self.proxyView else { return }
        print(self.proxyView?.outsideView.bounds ?? "")
    }
    ...
    private func setupViews() {
        ...
        insideGradientLayer.frame = CGRect(x: 0, y: 0, width: frameSide, height: frameSide)
        ...
        self.proxyView?.insideView.layer.cornerRadius = CGFloat( self.proxyView!.insideView.frame.height / 2 - 12)
    ...
    private func setupConstraints() {
        ...
        NSLayoutConstraint.activate([
            ...
            self.proxyView!.insideView.topAnchor.constraint(equalTo: (self.proxyView?.outsideView.topAnchor)!, constant: 8),
            self.proxyView!.insideView.bottomAnchor.constraint(equalTo: (self.proxyView?.outsideView.bottomAnchor)!, constant: -8),
            self.proxyView!.insideView.leadingAnchor.constraint(equalTo: (self.proxyView?.outsideView.leadingAnchor)!, constant: 8),
            self.proxyView!.insideView.trailingAnchor.constraint(equalTo: (self.proxyView?.outsideView.trailingAnchor)!, constant: -8),
    ...
    private func loadNib() -> TimerView {
        ...
        let view = bundle.loadNibNamed("TimerView", owner: nil, options: nil)?.first as! TimerView
        return view
    }
    ...
    private func setupTimerCircle() {
        ...
        let firstColorForInsideGradient = UIColor(displayP3Red: 0.615686297416687, green: 0.8078431487083435, blue: 1, alpha: 1).cgColor
        let secondColorForInsideGradient = UIColor(displayP3Red: 0.572549045085907, green: 0.6392157077789307, blue: 0.9921568632125854, alpha: 1).cgColor
        ...
        let path = UIBezierPath(arcCenter: center, radius: radius+5, startAngle: -0.25 * 2 * .pi, endAngle: 0.75 * 2 * .pi, clockwise: true)
        gradienOuterCircleMask.path = path.cgPath
        
        strokeGradientLayer.frame = CGRect(x: 0, y: 0, width: frameSide, height: frameSide)
        strokeGradientLayer.colors = [firstColorForInsideGradient, secondColorForInsideGradient]
        ...
        let pathForMask = UIBezierPath(arcCenter: center, radius: radius, startAngle: -0.25 * 2 * .pi, endAngle: 0.75 * 2 * .pi, clockwise: true).reversing()
